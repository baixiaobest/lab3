...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
        ...
        if ((entry_off=(f_pos-2)*OSPFS_DIRENTRY_SIZE)>=dir_oi->oi_size) {
            r = 1;
            break;
        }
        ...
        od = (ospfs_direntry_t*) ospfs_inode_data(dir_oi, entry_off); //get the entry data and cast to dir entry
        entry_oi = ospfs_inode(od->od_ino); //get the entry file's inode
        ...
        if (ok_so_far<0) { //end of directory
            break;
        }
        f_pos++;
	}
...
add_block(ospfs_inode_t *oi)
{
    ...
    if (indirIndex == -1) {
        ...
        if (oi->oi_direct[directIndex] == 0){//fail to add block
            return -ENOSPC;
        }
        ...
        uint32_t* indir_block_ptr = (uint32_t*) ospfs_block(oi->oi_indirect); // grab that indir block
        indir_block_ptr[directIndex] = allocate_block();
        //cannot allocate a new block for file
        if (indir_block_ptr[directIndex] == 0) {
            if (directIndex == 0) { //free the newly createdly indirect block
        ...
        if (directIndex==0) {
            ...
            if (double_indir_block_ptr[indirIndex]==0) {
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
    ...
    if (filp->f_flags&O_APPEND) {
        *f_pos = oi->oi_size;    //start writing from the every end of file
    }
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
    ...
    for (; current_inode_num<ospfs_super->os_ninodes; current_inode_num++) {
        ...
        if (current_inode->oi_nlink==0) { //grab the first empty inode
            break;
        }
    }
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
    ...
    //create new entry
    ospfs_direntry_t* symlink_entry = create_blank_direntry(dir);
    if (IS_ERR(symlink_entry)) {
        return PTR_ERR(symlink_entry);
    }
    //find empty inode first
    int current_symlink_inode_num = 2;
    ospfs_symlink_inode_t* current_symlink_inode;
    for (; current_symlink_inode_num<ospfs_super->os_ninodes; current_symlink_inode_num++) {
        current_symlink_inode = ospfs_inode(current_symlink_inode_num);
        if (current_symlink_inode->oi_nlink==0) { //grab the first empty symlink inode
            break;
        }
    }
    ...
    //feed the new entry with symlink inode
    symlink_entry->od_ino = current_symlink_inode_num;
